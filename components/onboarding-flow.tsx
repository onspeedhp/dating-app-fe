...
import { ArrowLeft, ArrowRight, X, Camera, User, Briefcase, Heart, ImageIcon, CheckCircle } from "lucide-react"
...
export function OnboardingFlow({ onComplete, user }: OnboardingFlowProps) {
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      case 1:
        ...
        else if (Number.parseInt(profileData.age) < 18) newErrors.age = "You must be 18 or older"
        else if (Number.parseInt(profileData.age) > 100) newErrors.age = "Please enter a valid age"
        if (!profileData.city.trim()) newErrors.city = "City is required"
        break
      case 2:
        if (!profileData.workTitle.trim()) newErrors.workTitle = "Job title or field of study is required"
        break
      case 3:
        if (!profileData.bio.trim()) newErrors.bio = "Bio is required"
        else if (profileData.bio.length > 160) newErrors.bio = "Bio must be 160 characters or less"
        break
      case 4:
        if (profileData.interests.length === 0) newErrors.interests = "Please select at least one interest"
        break
    }
  ...
  const addPhoto = () => {
    if (profileData.photos.length < 6) {
      const newPhotos = [
        ...profileData.photos,
        `/placeholder.svg?height=400&width=300&query=profile photo ${profileData.photos.length + 1}`,
      ]
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateProfileData({ workTitle: e.target.value })}
                  placeholder="e.g., Software Engineer, Psychology Student"
                  className="h-12 rounded-xl"
                />
                {errors.workTitle && <p className="text-sm text-destructive mt-1">{errors.workTitle}</p>}
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    if (profileData.workTitle.toLowerCase().includes("student")) {
                      updateProfileData({ school: e.target.value, company: "" })
                    } else {
                      updateProfileData({ company: e.target.value, school: "" })
                    }
                  }}
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <p className="text-muted-foreground">Write a short bio that represents you</p>
            </div>
            ...
            <div>
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <p className="text-sm text-muted-foreground ml-auto">{profileData.bio.length}/160 characters</p>
              </div>
            </div>
          </div>
        )
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <p className="text-muted-foreground">Select your interests to find better matches</p>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {interestOptions.map((interest) => (
                  ...
                  >
                    {interest}
                    {profileData.interests.includes(interest) && <X className="w-3 h-3 ml-2" />}
                  </Badge>
                ))}
              </div>
              <p className="text-sm text-muted-foreground mt-3">Selected: {profileData.interests.length} interests</p>
            </div>
          </div>
        )
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <p className="text-muted-foreground">Help others know your lifestyle choices</p>
            </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Do you drink alcohol?</p>
                    </div>
                    <Switch
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <p className="text-muted-foreground">Show your best self with up to 6 photos</p>
            </div>
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex items-center justify-between p-4">
        <Button variant="ghost" size="icon" onClick={handleBack} className="rounded-full" disabled={currentStep === 1}>
      ...
      <div className="p-6">
        {currentStep < STEPS.length ? (
          <Button onClick={handleNext} size="lg" className="w-full h-14 text-lg font-semibold rounded-2xl">
            Continue
            <ArrowRight className="w-5 h-5 ml-2" />
          </Button>
        ) : (
          <Button onClick={handleComplete} size="lg" className="w-full h-14 text-lg font-semibold rounded-2xl">
            Create Profile
