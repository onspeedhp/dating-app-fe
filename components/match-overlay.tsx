    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-sm border-0 bg-gradient-to-br from-primary/20 to-accent/20 backdrop-blur-sm">
        <CardContent className="p-8 text-center">
          ...
          <p className="text-muted-foreground mb-6">You and {profile.name} liked each other</p>

          {/* Profile Preview */}
          <div className="flex items-center justify-center gap-4 mb-8">
            <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-primary/20">
              <img src="/diverse-user-avatars.png" alt="Your photo" className="w-full h-full object-cover" />
            </div>
            <Heart className="w-6 h-6 text-primary fill-current" />
            <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-primary/20">
              <img
                src={profile.photos[0] || "/placeholder.svg?height=64&width=64&query=profile photo"}
                alt={profile.name}
          ...
          <div className="space-y-3">
            ...
            <Button variant="outline" size="lg" className="w-full h-12 rounded-2xl bg-transparent" onClick={onClose}>
              Keep browsing
            </Button>
          </div>
        </CardContent>
